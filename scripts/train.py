...
def main():
    ...
    group_col  = cfg["modeling"]["group_col"]
    ...
    pipe = make_baseline_pipeline(task, df.drop(columns=[c for c in reserved if c in df.columns]), reserved=[])
    ...
    for tr, te in gkf.split(df, y, groups):
        pipe.fit(df.iloc[tr], y.iloc[tr], **({"model__sample_weight": sample_weight.iloc[tr]} if sample_weight is not None and task=="classification" else {}))
        p = pipe.predict(df.iloc[te])
        preds.append(p); trues.append(y.iloc[te])
        if task == "classification":
            try:
                pr = pipe.predict_proba(df.iloc[te])[:,1]
            except Exception:
                pr = None
            probs.append(pr)
    ...
    out_df["y_true"] = y
    out_df["y_pred"] = pipe.predict(df)
    if task == "classification":
        try:
            out_df["y_prob"] = pipe.predict_proba(df)[:,1]
        except Exception:
            pass
